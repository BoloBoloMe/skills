---
name: use-worktree
description: 管理符合用户习惯的本地 Git worktree 布局：识别标准目录、选择/创建/删除/迁移 worktree、检查 dirty 状态并避免误改仓库。当用户要求操作本地 git repo、创建或清理 worktree、整理非标准 repo 布局、并行开发、避免改错分支时使用。
---
...
1. 用 `status.sh` 检查目标仓库布局。
2. 若布局标准：
   - 确认来源分支、目标分支；若未给出则询问。
   - 询问是否执行 `git fetch --all --prune`。
   - 用 `slug.sh <project> <source-branch> <target-branch>` 生成目录名。
...
迁移后执行 `git worktree repair` 修复登记路径，再用 `git worktree list` 与 `status.sh` 验证。
