...
当一个问题存在两个及以上合理选项时,在给出推荐答案前,逐选项分析三个维度:

- **影响面** -- 该选项会波及哪些文件/术语/ADR, 变更规模大致的量级
- **可逆性** -- 以后推翻该决定的代价: 受影响面是否集中, 是否有数据迁移, 下游依赖多少
- **对齐度** -- 与既有 `UBIQUITOUS_LANGUAGE.md`, ADR, 代码事实的一致性: 是否引入术语冲突或打破既有约束

用自然段落呈现,不要求固定模板.若你对影响面或可逆性的判断依据不确定,先探索代码库再给出分析.
