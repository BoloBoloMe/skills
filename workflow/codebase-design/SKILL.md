...
务必使用这些术语 - 不要用 component/service/API/boundary 等词代替, 语言一致性至关重要.

**Module** - 任何包含 interface 和 implementation 的实体. 刻意保持规模无关性: 可以是函数/类/包/跨层切片. _避免_: unit, component, service.

**Interface** - 调用者正确使用模块所需了解的一切: 类型签名, 不变量, 顺序约束, 错误模式, 必需配置, 性能特征. _避免_: API, signature (过于狭窄 - 仅指类型层面).

**Implementation** - module 内部的代码主体. 与 **Adapter** 不同: 一个事物可以 implementation 很大但 adapter 很小 (例如 Postgres 仓库), 也可以 implementation 很小但 adapter 很大 (例如内存 fake). 讨论重点在 seam 处的替换角色时用 **Adapter**; 否则用 **Implementation**.

**Depth** - interface 的杠杆效应: 调用者或测试者每学习一单位 interface 所能触发的行为量. 大量行为隐藏在较小 interface 背后时, 该 module 是 **deep module**; interface 复杂度几乎与 implementation 相当时, 该 module 是 **shallow module**.

**Seam** _(Michael Feathers)_ - 无需直接编辑代码即可改变行为的地方; 模块接口所在的位置. Seam 的位置本身就是一个设计决策, 与它背后的内容无关. _避免_: boundary (与 DDD 的限界上下文冲突).

**Adapter** - 在 seam 处满足 interface 的具体事物. 它描述 *角色* (它填补的空缺), 而非实质 (它内部的东西).

**Leverage** - 调用者从 depth 获得的收益: 每单位 interface 功能都更强大. 一个 implementation 即可在 N 个调用点和 M 个测试中产生效益.

**Locality** - 维护者从 depth 获得的收益: 变更, bug, 知识, 验证集中在一处, 而非散布在调用者中. 一次修复, 处处有效.
...
- **删除测试.** 想象删除该模块. 如果复杂度消失, 它是透传层. 如果复杂度在 N 个调用者中重现, 说明它有价值.
- **Interface 是测试界面.** 调用者和测试者都经过同一道 seam. 如果想测试 Interface 之外的内容, 那么这个模块的结构可能就不合适.
...
- **Depth 作为 implementation 行数与 interface 行数之比** (Ousterhout): 奖励 implementation 堆行数. 我们使用 depth-as-leverage.
- **"Interface" 作为 `interface` 关键字或类的公共方法**: 太狭窄 - 这里的 interface 包含调用者必须知道的所有事实.
