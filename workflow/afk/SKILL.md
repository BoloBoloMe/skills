...
仓库不维护 worker/reviewer prompt 模板. 每次启动新子代理前, 父会话根据本轮任务和执行现场编写专用 system prompt. 不引用角色文件, 不把上一轮 prompt 原样复用于不同任务. resume 同一子代理时沿用其原 system prompt, 并在恢复消息中补充新增事实.
...
步骤文件由 `to-execution-spec` 在切片确认后一次性生成. AFK 不生成或修改步骤定义.
...
开始前调用 `decision-ledger` 了解账本规则, 再读取 PRODUCT, TECHNICAL, EXECUTION, 当前 issue 和 DECISIONS (如存在).
...
**伪同步调度子代理**: 以异步+后台运行的方式启动单个子代理, 然后再阻塞等待子代理完成任务. 这完全是多此一举, 引入了额外的开销. 避免办法是直接同步调用单个子代理.
