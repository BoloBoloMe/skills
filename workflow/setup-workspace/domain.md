...
如果这些文件不存在,**静默继续**.不要提示它们缺失;不要一开始就建议创建它们.生产者 `grill-with-docs` skill 会在术语或决策真正被澄清时,按需懒创建这些文件.
...
如果你需要的概念还不在 glossary 中,那就是一个信号--要么你正在发明项目并不使用的语言(请重新考虑),要么确实存在缺口(为 `grill-with-docs` skill 记录下来).
