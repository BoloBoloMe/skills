...
- **Spec**: 代码是否完整实现了原始 issue / PRD / spec?
...
1. 我作为参数传入的路径或内容.
2. commit message 中的 issue 引用 (`#123`, `Closes #45`, GitLab `!67` 等). 若仓库存在 `docs/agents/issue-tracker.md`, 按其中流程获取.
3. `docs/`, `specs/` 或 `.scratch/` 下与分支名或功能名匹配的 PRD/spec 文件.
...
若运行时支持子 agent, 调用 `pi-subagents` skill, 确认存在可用的 `reviewer` 子 agent, 然后并行启动 **Standards** 和 **Spec** 两个 reviewer. 若运行时不支持子 agent, 没有可用 reviewer, 或并行启动失败, 父会话按同一输入契约先运行 Standards 再运行 Spec, 最终格式不变.
