...
1. **确定目的地.** 与我敲定这次 Probe 最终要得到什么, 必须是高清晰度目标; 不够清晰就调用 `grilling` skill 盘问我 `目标是什么`, 不深入 `如何做到` 的实现细节, 不使用 *反方攻击*, 直到目标清晰度足够为止.
   完成标准: 目的地已被几句话高清晰度描述.
2. **侦查方向.** 全面梳理, 从广度入手: 不限单一方向, 辐射整个空间, 选定几个你认为最具价值的方向, 每个方向都派遣一名子代理研究可行性/成本/收益, 多个子代理并行执行; 向我展示每个方向的研究结论, 每个方向都附上你的看法, 等待我选定方向. 我否决全部方向 → 回 step 1 重敲目的地. 各方向研究结论在落盘时记入 ROADMAP 笔记 — 含被选方向的结论和未选方向的排除理由.
...
5. **落盘.** 询问我是否确认落盘: `是` → 按 [TEMPLATES.md](TEMPLATES.md) 创建 ROADMAP.md 和 MILESTONE-NN.md, 画阻塞连线图; `不` → 等待我的下一步指示.
   完成标准: ROADMAP.md 六区段完整; 方向侦查结论已记入笔记; 每个 Milestone 文件头三字段正确; 连线图反映阻塞关系; 前沿列表准确.
6. **停止.** 绘制独占一个会话, 禁止在本会话开始解决 Milestone.
