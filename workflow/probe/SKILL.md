...
4. **绘制地图.** 调用 `present` skill 用 HTML 向我汇报 Roadmap. 路线, 目的地, 边界, 迷雾, 范围外, 里程碑都适合呈现在 "地图" 上 — 大胆画一张信息丰富的真地图. HTML 固定命名 ROADMAP.html, 落盘到 [产物结构](TEMPLATES.md) 约定的 Roadmap 目录 (此时目录可能尚不存在, 提前创建) — 目录即展示会话键, 重画覆盖同名文件, 在同一浏览器窗口原地刷新.
