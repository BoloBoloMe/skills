...
1. **确定目的地.** 与我敲定这次 Probe 最终要得到什么, 必须是高清晰度目标; 不够清晰就调用 `grilling` skill 盘问我 `目标是什么`, 不深入 `如何做到` 的实现细节, 不使用 *反方攻击*, 直到目标清晰度足够为止. 向我展示你对目的地的理解, 等待我确认.
...
4. **绘制地图.** 调用 `present` skill 用 HTML 向我汇报 Roadmap. 路线, 目的地, 边界, 迷雾, 范围外, 里程碑都适合呈现在 "地图" 上 — 大胆画一张信息丰富的真地图. HTML 固定命名 ROADMAP.html, 落盘到 [产物结构](TEMPLATES.md) 约定的 Roadmap 目录 (此时目录可能尚不存在, 提前创建) — 目录即展示会话键, 重画覆盖同名文件, 在同一浏览器窗口原地刷新. 展示环境 (本地/远程 ssh/容器) 的判定与交付方式由 present 全权负责, 容器内按其容器分支交付.
   完成标准: 浏览器窗口已打开展示 Roadmap HTML.
5. **落盘.** 询问我是否确认落盘: `是` → 按 [产物结构](TEMPLATES.md) 创建 ROADMAP.md 和 MILESTONE-NN.md, 画阻塞连线图; `不` → 等待我的下一步指示.
   完成标准: ROADMAP.md 区段完整 (目的地, 笔记, 已关闭决策, 前沿, 未决迷雾, 范围外, 阻塞关系); 方向侦查结论已记入笔记; 每个 Milestone 文件头三字段正确; 连线图反映阻塞关系; 前沿列表准确.
