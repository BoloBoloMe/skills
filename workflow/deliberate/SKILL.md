...
完成标准: 产品目标, 非目标, 行为和验收标准无阻塞未知; 我已在会话中明确确认关键产品取舍; 每个关键分支都满足 grilling 的完成标准.

## 2. 技术层面

基于已关闭的产品决策, 继续盘问技术层面的问题: 先校准技术认知, 探索基线代码与契约, 选择技术方向; 再下沉到*模块设计*; 继而异常, 安全, 质量属性, 代码允许/禁止范围, 风险和停止条件; 最后以*测试设计*收口.

*模块设计*: 架构/模块/接口 的设计必须符合 `codebase-design` skill 的原则, 必须和我逐项确认设计决策, 未经确认禁止固定.
*测试设计*: 测试用例必须以`tdd` skill 的哲学作为设计约束, 逐条与我确认, 未经确认禁止固定.
