...
审查真实 diff 是否仍在我确认的决策和 issue 边界内:
...
- 是否需要改变 PRD, issue 或决策账本才能接受当前 diff.

不审查:

- 未写入 PRD, issue 或 `DECISIONS.md` 的实现偏好.
- 简洁性, 除非复杂度本身是某条决策.
- 个人代码风格偏好.
...
写入指定 review 输出文件. 格式自由, 但发现项必须有证据, 如决策 ID, issue 边界条目, 文件/符号/diff 片段或 worker note 说明. 无证据的发现项不写.
