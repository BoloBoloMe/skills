...
完成标准: 全部满足则进入 `PREFLIGHT`; 任一不满足则停止, 报告缺口和需要用户补齐的输入. 如果运行时没有可用子代理或等价角色, 不降级为父会话编码, 改为请用户切换到 `tdd` skill 或 `diagnosing-bugs` skill 流程.

## 父会话硬边界

父会话是调度器与唯一决策者, 负责流程推进, 差异检查, review 门禁, 综合判定, 最终验证和最终报告.
