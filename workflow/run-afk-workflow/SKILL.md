...
| worker 完成, 父会话已检查真实 diff, 通过 review gate, 并写 `diff-summary.md` | 默认调用 review-only-safe | `AFK-RECIPES.md#review-only-safe` |
| `review-synthesis.md` 存在可立即修复的 `accepted_now` | 调用 fix-only | `AFK-RECIPES.md#fix-only` |
| 子代理 runtime 状态与 artifact/diff 冲突 | 父会话先补验事实, 不自动重跑 worker | `AFK-RUNBOOK.md#runtime-信号冲突处理`, `AFK-RUNBOOK.md#失败恢复` |
...
- 写入阶段单写入者. review 阶段只读, 默认 foreground 单 reviewer. 需要拆分时串行执行, 不默认 3 async 并行.
...
- 子代理 task 必须用 `<AFK_ARTIFACT_DIR>/...` 绝对路径读取 artifact. 禁止依赖 chain 相对路径或 step 子目录.
- worker/fix 是硬 TDD 阶段. 修改生产代码前必须先有行为测试或可执行检查的 RED 失败证据, 再做最小 GREEN 实现, 必要时重构并复跑验证.
- worker/fix 不能静默绕过 TDD. 若测试文件不在 `allowed-files.txt`, 缺少测试接缝, 需求不可验证, 或无法得到可信 RED, 必须报告 blocker, 不得先改生产代码.
- worker/fix 必须继承 `manifest.yaml` 或 `validation-profile.yaml` 中的验证环境. 错误 JDK/错误 profile 的失败不能作为代码失败证据.
- worker/fix 最终回复必须包含可解析的 `acceptance-report` fenced block, 且包含 `tdd-cycles` 和 `tests-added-or-changed`. 代码已改但 acceptance parse failed 时, 父会话先按 artifact, 真实 diff 和验证命令补验.

## 父会话最小检查

- [ ] 写入阶段前已得到用户执行确认.
- [ ] `<AFK_RUN_DIR>` 和 `<AFK_ARTIFACT_DIR>` 已创建, task 中的 `AFK_RUN_DIR` 与 `chainDir` 一致, task 中的 `AFK_ARTIFACT_DIR` 是绝对路径.
- [ ] `manifest.yaml`, `baseline.txt`, `doc-pointers.md`, `allowed-files.txt` 已存在于 `<AFK_ARTIFACT_DIR>`.
- [ ] `allowed-files.txt` 已包含本 milestone 需要的测试文件. 若不能包含, preflight 必须记录为 TDD blocker, 不启动 worker/fix.
...
- [ ] 进入 review 前已写 `diff-summary.md`, `worker-result.md` 存在, 若有 `tdd-cycles.md` 则 reviewer 明确读取其绝对路径, 真实 diff 未越过 `allowed-files.txt`, `git diff --check` 通过, 聚焦验证已运行或 blocker 已记录.
