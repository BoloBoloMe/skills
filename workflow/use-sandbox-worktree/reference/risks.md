...
- **noVNC 无认证**: x11vnc `-nopw`, 6080 只发布到宿主回环 (见 SKILL.md 显示栈节), 门槛 = 本机账户或容器 ssh 凭据持有者 (隧道命令即交付物). 这不是零风险: 拿到容器 ssh 凭据的人同时拿到一个已登录浏览器的完全控制 (D040).
- **本机直通信任面 (D051)**: 挂 wayland socket 后容器内任意进程可在宿主机桌面开窗口/读剪贴板 (distrobox 同级信任), 已拍板接受; 网络白名单语义零变化 (socket 非网络通道).
