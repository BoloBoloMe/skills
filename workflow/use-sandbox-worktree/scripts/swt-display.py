...
"""swt-display: 容器显示栈通道检查 (原 M09 login-wall verify 迁家, ISSUE-04 → D041).

纯逻辑切片: RFB 3.8 客户端 (握手/消息/raw 帧解码/PPM/非黑占比),
websocket 帧解码 (RFC 6455 §5.2), framebuffer 合成.
编排切片: verify 子命令 — noVNC HTTP / ws 握手 / RFB banner / 空白基线 /
渲染基线 (非黑占比 0.2) / headless 回切.

退出码: 0 全过; 1 有检查未过; 2 传输层失败 (podman/cp/exec/超时).
生命周期 (容器起停) 归 swt; 本模块只做检查, 不创建/删除任何容器.
"""
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    + PPM 证据回宿主 (5) headed chromium 渲染 -> framebuffer 非黑超阈值
    (0.2) + PPM 证据 (6) headless 回切. 全过 rc 0, 检查失败 rc 1;
    podman 传输层失败 (cp/exec/超时) 上抛经 main 封装 rc 2, 不与检查
    结果混淆."""
    results: list[bool] = []
    ...
    cleanup_evidence = args.evidence_dir is None
    evidence_dir = args.evidence_dir or tempfile.mkdtemp(prefix="swt-display-evidence-")
    Path(evidence_dir).mkdir(parents=True, exist_ok=True)  # 用户传入可能不存在
    print(f"evidence-dir: {evidence_dir}")
    _podman(["cp", str(Path(__file__).resolve()),
             f"{args.name}:/tmp/swt_display.py"], timeout=120)
    ...
    # 未指定 --evidence-dir 时, 临时证据目录随检查结束清理 (不泄 /tmp)
    if cleanup_evidence:
        shutil.rmtree(evidence_dir, ignore_errors=True)
        print(f"[swt-display] 临时证据目录已清理: {evidence_dir} (需留存请传 --evidence-dir)")
...
def _podman(args: list[str], check: bool = True,
    ...
    """timeout 600s 依据: exec swt-vnc 类操作秒级; 本模块 verify 只在
    已运行容器上做检查, 最坏段是 chromium 渲染轮询 (~300s), 600s 约
    2 倍裕度."""
...
def discover_ports(name: str) -> dict[str, str]:
    ...
    优先 0.0.0.0/[::] 表项; 缺省回退任一表项 (回环发布 127.0.0.1 表项
    落 fallback, 消费方经 127.0.0.1 连接, 宿主侧网络栈对 lo 目标同样
    命中映射).
    """
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="容器显示栈通道检查")
    subparsers = parser.add_subparsers(dest="command", required=True)

    p_verify = subparsers.add_parser(
        "verify", help="通道检查: noVNC HTTP/ws/RFB banner/空白基线/渲染基线")
    p_verify.add_argument("--name", required=True, help="容器名")
