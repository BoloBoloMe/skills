...
`workflow/` 下的技能预期在目标项目仓库根目录工作.首次使用 `to-prd`,`to-issues`,`to-plan`,`triage`,`diagnose`,`tdd`,`improve-codebase-architecture` 或 `zoom-out` 前,由 `orchestrate` 按需路由到 `setup-workspace` 生成约定文档.
...
workflow skills 的默认入口和元编排器:接收工程类用户任务,按静态决策树在 `workflow/` skills 间路由,处理前置 `setup-workspace`,并支持多阶段顺序编排.路由范围包括代码评审(`code-review-with-me`),执行计划(`to-plan`)等新增技能.

入口文档:[`workflow/orchestrate/SKILL.md`](workflow/orchestrate/SKILL.md)

### workflow/run-afk-workflow

`run-afk-workflow` 是 `orchestrate` 管辖下的 AFK 阶段入口. 当 `orchestrate` 已完成任务类型判断并符合 AFK 调用条件, 且当前阶段需要只读代码库探索以压缩上下文, 或需要已批准计划的 AFK 编码执行, diff 后 review, accepted finding 修复或恢复时, 用它按运行时无关角色契约启动 implementation, review, recovery 角色, 并保留父会话最终决策权. 入口见 [`workflow/run-afk-workflow/SKILL.md`](workflow/run-afk-workflow/SKILL.md), 细节见同目录 `CONTRACTS.md`, `RUNBOOK.md`, `RECOVERY.md`, `LIGHTWEIGHT-TEST-ONLY.md` 和 [`prompts/`](workflow/run-afk-workflow/prompts/).
...
### workflow/code-review-with-me

以"总-分-总"结构带领用户逐段交互式代码评审,产出评审日志和人审报告.

入口文档:[`workflow/code-review-with-me/SKILL.md`](workflow/code-review-with-me/SKILL.md)

### deprecated/hitl (已归档)

HITL 0.0.1 人在回路协议已移入 `deprecated/hitl/`,包含 `human-in-the-loop`(受控规划,批准,执行确认,审计链)和 `human-in-loop-brief`(方案评审简报生成).
...
Pi agent 本地配置文件,包含快捷键(`keybindings.json`),模型注册(`models.json`),扩展脚本(`extensions/`)和子代理配置示例(`pi-subagent-config-in-settings.json`).不随技能安装分发,仅供本机 pi 环境引用.
