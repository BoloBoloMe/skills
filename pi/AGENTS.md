...
当被要求使用子代理/subagent 时, 先用 llm-select skill 选定子代理的模型和推理深度, 
再用 herdr 开新会话 (用法读 `herdr --skill` 输出), 新会话开新标签页, 标签名 `S-<子代理名>-<序号>`; 
确认现在使用的是什么 agent, 如果是 pi , 那么先读 `~/.pi/agent/herdr-pi.md`.