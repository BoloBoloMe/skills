...
- 最终全量: `~/.local/bin/uv run --with pytest python -m pytest general/present/tests -q` → 5 failed (browser_session 本机缺 playwright, 既有环境差异, 见 U-005) + 101 passed + 1 skipped (browser integration 需 PRESENT_REAL_BROWSER).
...
- 本会话: U-004 测试命令改 `uv run --with pytest` (本机无根 pyproject/无 pytest) / U-005 browser_session 5 例失败定为环境差异不修 / U-006 子代理模型选型 (后被用户改为 flash 执行+glm-5.3 审核) / U-007 add-dir 端点回写 server.json roots (审核 S1, 消除 D005 矛盾) / U-008 不可读挂载 listing 语义 (全不可读 404, 部分 200 子集) / U-009 控制面放行 loopback 或源==bind (修 B1 回归: bind 具体 IP 时 CLI 自身 ping 被 403) / U-010 /__control__/* loopback 下不回落静态 (遮蔽兑现).
...
- browser_session 5 例: 是否补装 playwright 由用户定.
- `uv run python sync-to-pi.py` 是否执行由用户定.
