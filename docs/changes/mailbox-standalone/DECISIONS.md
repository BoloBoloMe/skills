...
### D021 取信守护标记绑定会话身份 (细化 D008)
- 状态: 当前有效 (用户 2026-09-24 实地验收时裁决)
- 约束性: 必须遵守
- 内容: 实地验收发现 D008 "session_start 见标记即自动恢复" 的字面实现导致任何新开 pi 会话都自动拉起取信守护 (用户原话: 启动取信的只有明确执行了取信命令的会话). 细化: listen.json 标记记录启动它的 pi 会话身份; session_start 仅当当前会话身份与标记一致 (同一会话的续接/重启) 时自动恢复守护; 新会话永不自动监听, 也不得覆盖/清除他人标记; /mail-listen start 遇他活会话标记仍软提示后接管 (D008); stop 仅清除自己拥有的标记; 标记进程已死视为陈旧, start 可接管. 若 pi 扩展 API 无法获得跨重启稳定的会话身份, 执行者停止并上报, 不得用 cwd 等近似物替代.
- 实际影响: ISSUE-15 实现
