...
- 验收记分 (用户本机实测): AC-001 注入唤醒 ✓; AC-002 忙时排队 ✓; AC-003 同会话续接自动恢复 ✓ (D021 新语义); AC-004 软提示 ✓ (真实触发); AC-005 回执不打断 ✓ (30s 静默 + 总览两条回执: 送达/已读); AC-006/007/008 分行回执与投递态 ✓; AC-011 邻居查询/删除 ✓ (清 3 条死邻居); AC-023 自投自取 ✓; AC-024 自动迁移 ✓; AC-026 机器子命令 register/revoke ✓; D018 status 邻居数 ✓; D020 fleet.key 引导 ✓.
- 验收期发现并修复: ISSUE-14 (broken pipe), ISSUE-15 (D021), 各经双轴评审.
- 候选 issue: ISSUE-16 (守护生命周期两限制 + relay 面堆栈), 未排期.
- 未验收 (外部条件): AC-017/AC-027 (需真容器 birth), AC-031..035 (需 yoga 更新代码后跨设备实测), AC-028..030 (随容器/birth 场景).
- 环境: 用户 serve 常驻 (38417), pi 会话守护运行中; session-prune 已恢复; test-peer 已注销.
