...
- 多容器共享母体互不影响的 resume/terminate/nft → `TestTS210NftMerge.test_second_container_rules_stay_in_each_container_netns` + `TestTS301Terminate.test_multiple_containers_require_name_and_remove_only_selected_network_rule` + `TestTS5Resume.test_ts507_resume_merge_preserves_sibling_network_rule` → nft `list table inet swt` 逐容器 SandboxKey 核对, `podman inspect` 确认兄弟容器仍在, resume 后各自规则均保留 → 通过.
