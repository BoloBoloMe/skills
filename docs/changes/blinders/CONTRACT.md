...
- 现有 worker, reviewer prompt 已重构为静态角色定义 (WORKER.md, REVIEWER.md), 父会话不读 prompt, 通过 task 参数注入路径.
- 现有 AFK 产物规范不变 (产物目录, 文件命名).
...
- to-issues 父会话执行 to-issues 时, 对适合 AFK 的 issue, 按 step-gen-guide 在 issue 产物目录生成全部步骤文件 (`step-01.md` ~ `step-15.md`) 和初始 `_current.md` (内容 `step-01.md`).
...
| D005 | step-gen-guide 位于 to-issues/references/, to-issues 按需加载 |
...
- `workflow/to-issues/references/step-gen-guide.md` — 新建. 被 to-issues 读取, 生成步骤文件.
- `workflow/run-afk-workflow/prompts/WORKER.md` — 新建. worker 静态角色定义.
- `workflow/run-afk-workflow/prompts/REVIEWER.md` — 新建. reviewer 静态角色定义.
- `workflow/to-issues/SKILL.md` — 步骤 6 (发布议题) 增加: AFK issue 时读取 step-gen-guide 并生成步骤文件.

prompt 文件重构: WORKER-IMPLEMENT.md + WORKER-FIX.md → WORKER.md; REVIEWER-CORRECTNESS.md + REVIEWER-DECISION-BOUNDARY.md → REVIEWER.md.

## 允许范围

- 可修改: `workflow/run-afk-workflow/SKILL.md`, `workflow/to-issues/SKILL.md`, `workflow/run-afk-workflow/prompts/` 下文件.
- 可新建: `workflow/to-issues/references/step-gen-guide.md`.
...
- 可观察: to-issues 执行后, issue 产物目录下存在 `_current.md` 和 15 个 `step-NN.md` 文件.
