...
(绘制会话拍板 — 见上方路线侦查结论与下方里程碑清单)

## 前沿

<!-- 开放 + 已解除阻塞 + 未被认领的 Milestone -->
- [MILESTONE-01](MILESTONE-01.md) — `deliberate` — 信箱通道设计盘问: 队列格式 / 消息类型与审批边界 / 常驻会话形态 / 多容器
...
- 信箱实时性: 先定时拉 (5-10s 间隔), 长连推送 (inotifywait 流式 / ssh -R) 升级 — 实时性诉求暴露时回访
