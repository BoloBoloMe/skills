...
## UD-13 锁定仅冷启动生效, 复用路径 flag 与实例锁定态不一致时告警
- 问题: 已存活实例上重复 `start --fixed-port` 走幂等复用, 锁定态不会补进既有实例 (锁定只在冷启动持久化); 双轴评审均指出该静默无操作有运行时违 D001 的风险.
- 决策: 维持 "锁定仅冷启动生效" 语义 (不实现复用升级锁); 复用路径遇请求锁定但存活实例未锁 → 输出 warning 明示; SKILL.md 容器分支写明该语义.
- 理由: 复用升级锁需改存活实例状态文件, 侵入大于收益; 容器分支流程首次 start 即带锁, 不一致只出现在异常路径, 告警足够.
- 影响: run_start 复用分支加锁定态比对 + warning; SKILL.md 一句说明.
- 风险: 低; 异常路径现在有声.
