...
文档只供 AI 使用. 我不通过阅读文档理解或批准方案. 影响产品, API, 架构, 范围, 风险或验证的决定, 必须在 `grill-with-docs` 会话中向我解释并确认.
...
先关闭产品分支, 再关闭技术分支. 一次只问一个问题. 每项关键选择在会话中说明含义, 影响和推荐, 得到确认后才写入 `DECISIONS.md`/ADR/领域语言.

结束时直接在会话中回顾产品结果, 技术选择, 非目标, 风险和验证方式. 不要求我阅读任何文档.

### 4.2 to-product-spec

把已确认产品结果写入 `PRODUCT.md`. 不做访谈或新增决定. 缺口或冲突退回 `grill-with-docs`.

完成后只在会话中报告路径, 待验证事实和阻塞, 不展示全文.

### 4.3 to-technical-spec

探索代码事实, 将已确认设计写入 `TECHNICAL.md`. 不新增产品/API/架构/范围决策. 缺口或冲突退回 `grill-with-docs`.
...
- 产品/API/架构/范围语义变化退回 `grill-with-docs`, 新建或替代决策 ID, 再更新对应 Spec.
