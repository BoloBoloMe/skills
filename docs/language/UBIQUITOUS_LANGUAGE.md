...
独立 skill `mailbox` 提供的 agent 间传话通道, 双向: 任意 Session (容器/host/设备) 既可投信也可取信. 信件全内存 (信箱重启即丢排队信, 发信方自行重发), 投递语义为租约式 (借出/超时收回重投/确认). 拓扑为 mesh 全互联 — 每台设备跑本机信箱, 非本机收件人的信洪泛转发给邻居 (seen-id 防环, 排除来源邻居), 邻居不可达时内存暂存重试. 无中心单点. LLM 中转与信箱同住单脚本 (二合一). 信箱服务与沙盒工作树无隶属关系, swt 只使用不实现.
_避免_: 消息队列 (未含 mesh 与内存语义), 单体服务 (旧形态), swt 信箱 (已独立, 不再隶属 swt)

**取信会话** (已重构, 见 ADR-0014/ADR-0015):
设备侧处理**信箱**来信的专职 agent 会话 (不限于 pi). 两种形态: pi 会话内由**取信守护**承担 (扩展代劳); 其他 agent (codex/kimi/裸终端) 手动前台跑取信脚本 — LLM 循环调用, 来信经脚本输出唤醒处理, 处理完再调 (自动回执上一条), host 上开在当前会话 tab 的窗格, 其他设备开固定命名 tab (`S-swt-relay-1`), 手动首启不自启.
_避免_: 接收端 (旧方案残留), pi 专属会话 (现已跨 agent)

**取信守护** (新增, ADR-0015):
mailbox skill 的 pi 扩展为当前 pi 会话跑的后台取信循环 (`/mail-listen`): 阻塞取信脚本逐封取信, 来信注入当前会话唤醒 LLM (agent 忙则排队, 闲才唤醒), 闲后再取下一封 (顺带自动回执上一封). 不阻塞前台, 状态落 listen.json 自动恢复, 多会话同开只软提示不拦截.
_避免_: 后台轮询 (未含注入唤醒与回执时序语义), 推送监听

**回执信** (新增, mailbox-standalone D012):
信箱服务端生成的 best-effort 事件通知信 (送达/已读/失败三种), 反向路由给原发件人; 语义钉死 “收到 = 确定发生, 缺席 = 未知”. 带标记不递归, pi 侧不唤醒 LLM 只汇入状态.
_避免_: 投递确认 (暗示强一致保证), 回执 (未含 best-effort 语义)

**舰队密钥** (新增, ADR-0016):
信箱 mesh 的入群总钥匙 (fleet.key, 0600), 只经既有 ssh 信任锚一次性分发 (join-fleet); 节点凭它互证身份自动建邻居, 无它者一律拒入. 只作入群认证, 邻居链路密钥仍两两独立协商.
_避免_: 主密钥 (过泛), 配对码 (已否决的邀请码方案残留)
...
**守门脚本** (新增, ADR-0017):
navigate skill 的 roadmap.py: llm 读写 ROADMAP.json 的唯一通道 (save/delete/query), 负责校验并拦截非法写入 — 写后整文档 schema 校验, 失败不落盘. 路线图的格式正确性与必填信息齐备由它强制, 不依赖 llm 自觉.
_避免_: 路线图工具 (未含守门语义), 编辑器 (它是校验闸门)
...
开发者: 我投的信到底到没到?
领域专家: 看**回执信**: 收到送达回执就是进了对方队列; 什么都没收到就是未知 — 回执是 best-effort, 缺席不代表没到.
开发者: 两个 pi 会话都开了**取信守护**会怎样?
领域专家: 不做唯一性强制, 信散落到先等到的会话; 扩展会给软提示, 处不处理在我.
