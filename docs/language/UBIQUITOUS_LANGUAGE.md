...
**基础服务**:
**sandbox-worktree** 的 host 常驻单体 web 服务 (`swt-base-server.py`, Quadlet 驻留): LLM 中转 (吸收 llm-proxy) + **信箱** 二合一. 永远搭建在容器所在的宿主机上, 容器内地址恒为 `host.containers.internal` + 端口区间 (38417-38426, 启动绑首个空闲, `/__identity__` 端点供探测身份). 容器 key 与设备签名密钥分角色: 容器只能投信, 设备才能取信.
_避免_: 消息队列服务 (未含 LLM 中转与单体语义), 中转站 (llm-proxy 旧称, 已被吸收)

**信箱**:
**基础服务**中的容器侧→设备侧传话通道, 纯单向: 容器投信 (纯文本结构化消息), 设备侧**取信会话**长轮询阻塞取信. 无回程队列 — 设备回话走既有 ssh/herdr 通道. 消息存 host SQLite (容器删除不丢信), 按目标设备路由 (缺省发最近活跃设备), 全单播.
_避免_: 消息队列 (过泛, 未含单向与纯文本语义), 通知系统

**取信会话**:
设备侧处理**信箱**来信的常驻 pi 会话: 扩展在后台跑阻塞取信脚本, 来信经 triggerTurn 唤醒 LLM 处理. host 上开在当前会话 tab 的窗格, 其他设备开固定命名 tab (`S-swt-relay-1`), 手动首启不自启.
_避免_: 接收端 (旧方案残留, 未含 pi 会话与唤醒语义)
