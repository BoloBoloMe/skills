...
计划保存到：`docs/human-in-loop-execution/plans/<yyyy-mm-dd>-<任务概括>.md`。
...
先列文件结构和文件职责，再拆任务。每个任务的每步目标 2-5 分钟，包含精确文件路径、失败测试或验证命令、预期输出、最小实现、回归验证、提交或变更记录。
...
输出已保存计划路径、任务列表摘要、绑定的 HILP asset_ref、禁止越界项、自检结果和推荐执行方式。若发现蓝图无法拆分，停止并要求回到 HILP 变更重审或实施蓝图。
